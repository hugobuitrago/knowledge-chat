...
Na Fase 5, o Worker passou a renovar o lease entre etapas e batches. A transação final bloqueia novamente o job, confirma token e expiração, substitui os chunks do documento, atualiza documento/versão e conclui o job atomicamente. Encerramento durante uma chamada não libera o job: outro Worker o recupera após a expiração. Falha terminal marca documento e versão como `Failed`; retry manual também restaura esses estados.

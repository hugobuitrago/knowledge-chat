    ...
    IOptions<JobsOptions> options) : IIngestionJobQueue
{
    ...
    public async ValueTask<IngestionJobLease> RenewAsync(
        IngestionJobLease lease,
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(lease);
        ...
        {
            throw new IngestionJobLeaseLostException(lease.JobId, exception);
        }

        await dbContext.SaveChangesAsync(cancellationToken).ConfigureAwait(false);
        return lease with { LockedUntil = job.LockedUntil!.Value };
    }
        ...
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(lease);
        IngestionJob job = await GetOwnedJobAsync(lease, cancellationToken).ConfigureAwait(false);
        bool terminal = !isTransient || job.Attempts >= job.MaxAttempts;
        if (terminal)
        {
            (Document document, KnowledgeBaseVersion version) =
                await GetProcessingEntitiesAsync(job, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<(Document Document, KnowledgeBaseVersion Version)>
        GetProcessingEntitiesAsync(
            IngestionJob job,
            CancellationToken cancellationToken)
    {
        Document document = await dbContext.Documents.SingleAsync(
            candidate => candidate.Id == job.DocumentId &&
                candidate.TenantId == job.TenantId &&
                candidate.KnowledgeBaseId == job.KnowledgeBaseId &&
                candidate.VersionId == job.VersionId,
            cancellationToken).ConfigureAwait(false);

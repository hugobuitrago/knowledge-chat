...
Fundação de uma API RAG multi-tenant em .NET 10. O repositório contém um monólito modular com API e Worker implantáveis separadamente. Esta entrega implementa as Fases 0 a 4: fundação arquitetural, persistência PostgreSQL/pgvector, segurança máquina a máquina, upload streaming e fila de ingestão com leases. Chunking, embeddings, retrieval e geração ainda não foram implementados.
...
| `Embedding__Model` | sim | `development-placeholder` | identificador fixado na versão pendente; deve ser configurado antes da Fase 5 |
...
Nesta fase o Worker recebe a configuração e a implementação da fila, mas ainda não transforma o documento em chunks: o consumidor de conteúdo e embeddings pertence à Fase 5.

## Persistência

A migração inicial cria a extensão `vector` e nove tabelas:
...
As decisões estão em `docs/adr/0001` a `0007`. Classificação de dados, fronteiras de confiança, ameaças e controles presentes ou planejados estão em `docs/threat-model.md`.

## Limites desta entrega

Não há provisionamento HTTP de tenants/chaves, scanner antimalware, adapter de object storage de produção, chunking, geração de embeddings, ativação de versão, retrieval, chamada de LLM, `/metrics` Prometheus ou testes de carga. A fila controla leases e estados, mas o processamento do conteúdo começa somente na Fase 5.
